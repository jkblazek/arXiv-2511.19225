...
def avail_at_bprice_(i: int, y: float, M: Dict, ladders: Dict) -> np.ndarray:
    """
...
def sup_G_i_multi(i: int, M: Dict, ladders: Dict) -> Tuple[float, np.ndarray, float, Dict]:
    ...
    #    Availability is constant on (y_k, y_{k+1}) under QJC, so evaluate at y_k (strict '>').
    for k in range(len(steps) - 1):
        y_left, y_right = steps[k], steps[k + 1]
        if y_right <= y_left + tol:
            continue
        caps = avail_at_bprice_qjc(i, y_left, M, ladders)
        Z = caps.sum()
    ...
    for k in range(len(steps) - 1):
        yk = steps[k]
        Zk = avail_at_bprice_qjc(i, yk, M, ladders).sum()
        if theta_i_prime(i, Zk, M) >= yk - tol:
            k_star = k

    y_star = steps[k_star]
    caps = avail_at_bprice_qjc(i, y_star, M, ladders)
    Z = caps.sum()
    z_star = min(M["qbar"][i], Z)
